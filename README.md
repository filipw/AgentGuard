...
- **StackOne Defender prompt injection detection** - uses the [StackOne Defender](https://github.com/StackOneHQ/defender) fine-tuned multi-head MiniLM-L6 ONNX model (minilm-multihead-v5, ~22 MB, bundled in NuGet) for ML-based classification. Calibrated dual-head decision (`main >= threshold AND aux < veto`) keeps imperative-but-benign requests like "show me my orders" from being flagged. ~8 ms inference, fully offline. **No download required** - the model is bundled with `AgentGuard.Onnx`. Order 11 (default). Also supports two optional DeBERTa v3 rules at order 12 (separate download): a generic bring-your-own-model rule (`./eng/download-onnx-model.sh` fetches a third-party model by default) and **PIGuard** (`./eng/download-piguard-model.sh`), the latter far stronger on indirect / code-style injection (see [docs/rules-reference.md](docs/rules-reference.md#prompt-injection-detection-onnx---piguard))
